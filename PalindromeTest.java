...
class Manager implements Runnable{
	...
	public void run(){
		...
		PalindromeMiner workers[] = new PalindromeMiner[nthreads];
		  long runtimes[] = new long[nthreads];
	       ...
	       ExecutorService executor = Executors.newFixedThreadPool(nthreads);  
	        ...
	        for (int i = 0; i < nthreads; i++) {  
	        	tmax = tmin+incr;
	            workers[i] = new PalindromeMiner(tmin, tmax);  
	...
	private static int calculateRuntimes(PalindromeMiner workers[], long times[]){
		int total=0;
